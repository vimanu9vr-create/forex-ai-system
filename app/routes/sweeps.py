...
@router.get("/sweeps")
def sweeps(pair: str = "EURUSD"):
    candles = get_forex_intraday(pair)
    if not candles:
        return {"sweeps": [], "error": f"No candle data for {pair}"}
    ...
    all_sweeps = detect_buy_side_sweeps(candles, liquidity_zones)
    all_sweeps += detect_sell_side_sweeps(candles, liquidity_zones)

    return {"sweeps": all_sweeps}
